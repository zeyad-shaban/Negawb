...
def view_comment(request, comment_id):
    ...
    replies = Reply.objects.filter(comment = comment).order_by('-reply_date')[:150]
    if request.method=='GET':
...
def results(request):
    ...
    results = Category.objects.filter(Q(title__icontains=query) | Q(description__icontains=query))
...
def results_comment(request):
    query = request.GET.get('q')
    comments = Comment.objects.filter(Q(title__icontains=query) | Q(description__icontains=query))
    return render(request, 'categories/results_comment.html', {'comments':comments})
